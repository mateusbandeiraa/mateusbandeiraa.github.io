...
document.addEventListener("DOMContentLoaded", async function () {
    let trackPlayer = new Audio(`/80s-karaoke/assets/songs/song${urlTrackId}.mp3`);
...
function updateStatus(trackPlayer) {
    ...
    if (trackPlayer.paused && htmlStatusTextElement.textContent != "PAUSE") {
        ...
        htmlStatusIconElement.setAttribute("data", "/80s-karaoke/assets/icons/pause.svg");
    } else if (!trackPlayer.paused && htmlStatusTextElement.textContent != "PLAY") {
        htmlStatusTextElement.textContent = "PLAY";
        htmlStatusIconElement.setAttribute("data", "/80s-karaoke/assets/icons/play.svg");
    }
}
...
function fetchTrack(trackId) {
    return fetch(`/80s-karaoke/assets/songs/song${trackId}.json`).then((response) => response.json());
}
